import { ContextEntry, useCedarStore, withClassName, type CedarEditor as Editor } from 'cedar-os';
 ...
 */
function normalizeToArray(value: ContextEntry | ContextEntry[]): ContextEntry[] {
...
export const ContextBadgeRow: React.FC<ContextBadgeRowProps> = ({ editor }) => {
  ...
  const contextElements = React.useMemo(() => {
    ...
    contextKeysWithOrder.forEach(({ key, entries }) => {
      ...
      if (keyCollapseConfig && visibleEntries.length > keyCollapseConfig.threshold) {
        ...
        const collapsedLabel =
          keyCollapseConfig.label?.replace('{count}', visibleEntries.length.toString()) ||
    ...
    return elements;
  }, [additionalContext, mentionProviders, collapsingConfigs, renderContextBadge]);

  return (
    <div id="input-context" className="hidden">
      <div
